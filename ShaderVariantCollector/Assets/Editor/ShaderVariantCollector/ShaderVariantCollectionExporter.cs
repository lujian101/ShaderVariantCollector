...
public static class ShaderVariantCollectionExporter {
    ...
    static Dictionary<String, RendererParams> _CollectDynamicLightingMaterials() {
        ...
        var pathList = new DynamicRenderersSettings[] {
            new DynamicRenderersSettings {
                assetPathRoot = "Assets/Resources/Prefab/bg",
                wildcard = "bg_roleshow_*"
            },
        };
        try {
    ...
    static void _BuildShaderAssetBundles( List<String> shaders, BuildTarget buildTarget ) {
        var b = new AssetBundleBuild();
        var outpath = Application.dataPath + "/../Temp";
        var fileName = String.Format( "_temp_built_shaders_{0}.ab", buildTarget );
        ...
        b.assetBundleName = fileName;
        b.assetBundleVariant = String.Empty;
        b.assetNames = shaders.ToArray();
        var options = BuildAssetBundleOptions.StrictMode |
            BuildAssetBundleOptions.UncompressedAssetBundle |
            BuildAssetBundleOptions.DeterministicAssetBundle;
        CustomShaderPreprocessor.ProcessShaderCallback recorder = null;
        recorder = ( Shader shader, ShaderSnippetData snippet, IList<ShaderCompilerData> data ) => {
            Debug.LogFormat( "{0}: {1}, {2}, Name = '{3}'", shader.name, snippet.shaderType, snippet.passType, snippet.passName );
            ShaderVariantValidationCache.sharedInstance.AppendSnippet( shader,
                new ShaderVariantValidationCache.SnippetData {
        ...
        CustomShaderPreprocessor.OnProcessShaderCallback += recorder;
        var builds = new AssetBundleBuild[] { b };
        AssetBundleManifest manifest = null;
        try {
            ShaderVariantValidationCache.sharedInstance.Reload();
            ShaderPreprocessor.OnBeginBuildAssetBundles( outpath, builds );
            BuildPipeline.BuildAssetBundles( outpath, builds, options, buildTarget );
        } finally {
            ShaderPreprocessor.OnEndBuildAssetBundles( outpath, builds, manifest );
            CustomShaderPreprocessor.OnProcessShaderCallback -= recorder;
            ShaderVariantValidationCache.sharedInstance.Save();
        }
    }

    static void BuildShaderAssetBundles( List<String> shaders ) {
        var buildTarget = EditorUserBuildSettings.activeBuildTarget;
        var window = EditorWindow.GetWindow( typeof( CustomMessageBox ), true, "Build Shader AssetBundles" ) as CustomMessageBox;
        if ( window != null ) {
            EditorUtility.ClearProgressBar();
            window.Info = "Build Shader AssetBundles";
            window.minSize = new Vector2( 320, 100 );
            window.maxSize = window.minSize;
            window.Show();
            window.Buttons = new String[] { "Build", "Dump", "Cancel" };
            window.OnGUIFunc = () => {
            ...
            window.OnButtonClicks = new Action<int>[] {
                sel => {
                    _DumpList();
                    _BuildShaderAssetBundles( shaders, buildTarget );
                },
                sel => _DumpList(), null
            };
        }
    ...
    /// <summary>
    /// 把选中的Shader以及对应变体集打到AssetBundle中，用于快速验证Shader是否正确
    /// </summary>
    [MenuItem( "Tools/Shader/Build Selected Shaders Assets" )]
    static void BuildSelectedShaderAssets() {
        var shaders = new HashSet<String>();
        var files = EditorUtils.GetSelectedAssetPathList( typeof( Shader ), null );
        for ( int i = 0; i < files.Count; ++i ) {
            if ( files[ i ].EndsWith( ".shader", StringComparison.OrdinalIgnoreCase ) ) {
                _CollectShaderDependencies( files[ i ], shaders );
            }
        }
        if ( shaders.Count == 0 ) {
            UDebug.LogError( "No shader assets selected." );
            return;
        }
        BuildShaderAssetBundles( shaders.ToList() );
    }
    ...
    [MenuItem( "Tools/Shader/Build All Shaders Assets" )]
    static void BuildAllShaderAssets() {
        var shaders = new HashSet<String>();
        var all = ShaderUtil.GetAllShaderInfo();
        for ( int i = 0; i < all.Length; ++i ) {
            var shader = Shader.Find( all[ i ].name );
        ...
        if ( shaders.Count == 0 ) {
            UDebug.LogError( "No shader assets in project." );
            return;
        }
        BuildShaderAssetBundles( shaders.ToList() );
    }
    ...
    internal class ShaderVariantValidationCache {
        ...
        const String FilePath = "Assets/ToolOutput/ShaderVariantValidationCache.json";
    ...
    static ShaderVariantCollection LoadOrCreateSVC( String shaderPath, Dictionary<String, SVCWithList> svcRecords, out KeyValuePair<String, SVCWithList> kv ) {
        ...
        if ( FileUtils.CreateDirectory( shaderVariantsPath ) ) {
            ...
            if ( !svcRecords.TryGetValue( shaderVariantsPath, out newSVC ) ) {
                ...
                shaderVariantsPatchPath = shaderVariantsPatchPath.Insert( 0, "Assets/ShaderVariants/__patches/" );
    ...
    static IEnumerator ShaderVariantsCollector( bool includeScenes, List<String> _excludeShaders, List<String> _updateOnlyShaders, bool debugBypass = false, bool useShaderVariantValidationCache = true ) {
        ...
        var realtimeScenePath = "Assets/Developement/LightEnvs/realtime.unity";
        var lightEnvPathList = new List<String>(
            new String[] {
                realtimeScenePath,
                "Assets/Developement/LightEnvs/subtractive.unity",
                "Assets/Developement/LightEnvs/shadowmask.unity",
                "Assets/Developement/LightEnvs/shadowmask_directional.unity"
            }
        );
        EditorSceneManager.OpenScene( LevelEditor.BlackholeScenePath );
        ...
        for ( int envIndex = 0; envIndex < lightEnvPathList.Count; ++envIndex ) {
            ...
            var lightEnv = UnityEngine.Object.FindObjectOfType<Developement.LightEnv>();
            if ( lightEnv == null ) {
                continue;
            }
